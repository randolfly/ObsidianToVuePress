...
namespace ObsidianToVuePressTest.LibTest
{
    [TestClass]
    public class MdLibTest
    {
        public readonly static string PaperText = @"aliases: 
tags: [公理系统, logic, set-theory]
date created: July 28th 2022, 6:51:48 pm
date modified: August 18th 2022, 6:56:44 pm
title: Chap1 基础集合论
star: 5
...
";
        public readonly static string MathText = @"\begin{cases}
	{\color[RGB]{240, 0, 0} u|_{t=0}}=\phi \left( x \right)\\
	\frac{\partial u}{\partial t}|_{t=0}=\psi \left( x \right)\\
\end{cases}";

        [TestMethod]
        public void ReplaceMathText_NormalTest()
        {
            string replaceMath = MathText.ReplaceMathText();
            Console.WriteLine(replaceMath);

            string targetText = @"\begin{cases}
	{\color{red}{u|_{t=0}}}=\phi \left( x \right)\\
	\frac{\partial u}{\partial t}|_{t=0}=\psi \left( x \right)\\
\end{cases}";
            bool isSame = string.Equals(targetText, replaceMath, StringComparison.OrdinalIgnoreCase);
            Assert.IsTrue(isSame, "Check if the color regex parser error");
        }
    }
}
